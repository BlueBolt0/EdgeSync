...
class _CameraAppState extends State<CameraApp>
  ...
  Future<void> _initServices() async {
    _faceDetector = FaceDetector(
      options: FaceDetectorOptions(
        ...
        performanceMode:
            FaceDetectorMode.fast, // Use fast mode for older devices
      ),
    );

    _harmonizerService = HarmonizerService();
    _voiceCommandService = VoiceCommandService();
    await _voiceCommandService.init();
    _voiceCommandService.actionStream.listen((action) {
      _handleVoiceCommand(action);
    });
    _voiceCommandService.listeningStateStream.listen((isListening) {
      if (mounted) {
        setState(() {
  ...
  void _detectDevicePerformance() {
    if (Platform.isAndroid) {
      try {
        ...
        _processInterval = _isOldDevice
            ? kOldDeviceInterval
            : kNewDeviceInterval;

        print(
          'Device performance detected: ${_isOldDevice ? "Old" : "New"} device',
        );
        print(
          'Processing interval set to: ${_processInterval.inMilliseconds}ms',
        );
      } catch (e) {
        _isOldDevice = true;
      ...
      if (mounted) {
        showDialog(
          ...
          builder: (context) => HarmonizerDialog(
            key: ValueKey(
              'harmonizer_${DateTime.now().millisecondsSinceEpoch}',
            ),
            imagePath: imagePath,
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => GalleryScreen(
          mediaPaths: _mediaPaths.reversed.toList(),
          initialIndex: 0,
        ),
      ),
    );
  ...
  void _handleVoiceCommand(CameraAction action) {
    switch (action) {
      ...
      case CameraAction.stopVideo:
        _stopVideoRecording();
        break;
      case CameraAction.switchCamera:
        _switchCamera();
        break;
      case CameraAction.toggleFlash:
        _toggleFlash();
        break;
      case CameraAction.toggleHarmonizer:
        setState(() {
          _harmoniser = !_harmoniser;
        });
        break;
      case CameraAction.togglePhotoMode:
        setState(() {
          _isPhotoMode = true;
        });
        break;
      case CameraAction.toggleVideoMode:
        setState(() {
          _isPhotoMode = false;
        });
        break;
      ...
      body: SafeArea(
        child: Stack(
          children: [
            ...
            // COUNTDOWN OVERLAY
            if (_countdown > 0) AnimatedCountdownWidget(countdown: _countdown),
                        ...
                        _buildTopIconButton(
                          ...
                          onTap: () {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (context) =>
                                    const HarmonizerSettingsScreen(),
                              ),
                            );
                          },
                        ),
